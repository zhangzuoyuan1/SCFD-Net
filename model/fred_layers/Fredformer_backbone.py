...
from model.fred_layers.RevIN import RevIN
from model.fred_layers.cross_Transformer_nys import Trans_C as Trans_C_nys
from model.fred_layers.cross_Transformer import Trans_C
# Cell
import torch
...
class Fredformer_backbone(nn.Module):
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        z1 = z.real #复数的实部
        z2 = z.imag #复数的虚部
#         在傅里叶变换（FFT）中，复数结果通常包含实部（real）和虚部（imag），这两部分在信号处理和深度学习中有重要意义：

# 实部（z.real）：表示信号在频域中的余弦分量（幅度与相位信息的一部分）。
# 虚部（z.imag）：表示信号在频域中的正弦分量（相位信息的另一部分）
